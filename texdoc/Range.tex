...
The context pointers passed as \verb|arg| to lazy adaptors are also borrowed.
They must remain valid while any cursor or terminal operation can invoke the
...
Range operations that return \verb|int| use signed status values so that normal
termination cannot be confused with an error:
...
Library-detected argument and protocol failures are returned as negative error
codes and are normally also sent to an error function.  New ranges initially
use the value of \verb|iError.RaiseError| in effect when they are created.
Called on the outermost handle, \verb|SetErrorFunction| installs a handler on
the complete pipeline and returns the former handler.  Passing \Null as the
new function queries the current handler without changing it.  Invalid calls
that have no usable range instance are reported through the global
\verb|iError| handler.

A negative result returned by an application callback is already a complete
error report from that callback.  The range propagates it unchanged and does
not invoke the range error function a second time.  Applications must always
check negative return values, regardless of the configured reporting callback.
...
destroyed.  Passing a \Null allocator selects \verb|CurrentAllocator|.
...
The callback and \verb|arg| supplied to \verb|Filter|, \verb|Transform|,
\verb|TakeWhile| or \verb|DropWhile| are stored in the lazy recipe; their
lifetimes must cover every evaluation that can call them.
...
caller-owned accumulator.  If evaluation later fails, earlier accumulator
updates are not rolled back. \\\hline
\verb|FindIf| & Copies the first matching value to \verb|result|.  Returns one
if found, zero if absent, or a negative error.  The caller must provide at
least \verb|GetElementSize(range)| writable bytes. \\\hline
...
destroys the complete recipe and rejects a retained inner handle.  In contrast,
\verb|GetElementSize| validates only that its argument is non-null; callers
must still obey the unique-ownership rule and query the current outermost
handle.
